/* * * * * * * * * * * * * * * * * * * * * * * * * * *
 * mcsubLIB.c
 * Simulación Monte Carlo del ritmo de fluencia F y el flujo de escape J
 * en un medio semi-infinito como el tejido biológico
 * con una frontera medio_externo/tejido.
 *
           ...
           short PRINTOUT) 
{
	...
	CNT = 0;
	mut    = mua + mus; //Coeficiente total
	albedo = mus/mut; //Albedo
	...
	*/
	for (iphoton=1; iphoton<=Nphotons; iphoton++) 
	{
		...
		*/
		...
		{
			/*
			* Lanzar en (r,zz) = (radius*sqrt(rnd), 0).
		...
		*/
		do 
		{
			...
			*****/
			...
			/* Actualizar posiciones, de acuerdo al paso y los cosenos directores */
			x += s*ux;
			y += s*uy;
			z += s*uz;
		
			/* ¿El fotón escapó en la superficie? ... z <= 0? <SECCIÓN DE ESCAPE FOTÓN>*/
			if ( (boundaryflag == 1) & (z <= 0)) 
			{
				rnd = RandomGen(1,0,NULL); //Generar otro número aleatorio
				/* Revisar la reflectancia de Fresnel en la frontera de la superficie */
				if (rnd > RFresnel(n1, n2, -uz, &uz1))
				{  
					/* El fotón escapa en un ángulo externo, uz1 = cos(angle) */
			...
			if (photon_status  == ALIVE) 
			{
				...
				*****/
				...
				/* Depositar la potencia en las coordenadas cilindricas z,r */
				...
				*****/
				/* Muestreo para costheta 
					...
					el valor sintheta.
				*/
				rnd = RandomGen(1,0,NULL); //Genera un número aletorio nuevo
				//Calcular el ángulo con el que se deflecta el fotón de acuerdo con el coeficiente de anisotropia
				if (g == 0.0) //Cuando es cero
					costheta = 2.0*rnd - 1.0;
				else if (g == 1.0) //En caso de que sea 1
					costheta = 1.0;
				else //En otro caso, que es el que generalmente se utiliza
				{
					//Se calcula mediante la función de Henyey-Greenstein
...
double **AllocMatrix(short nrl,short nrh,short ncl,short nch)
{
	...
	if (!m) nrerror("Fallo asignación 1 en matrix()");
		m -= nrl;
	for(i=nrl;i<=nrh;i++) 
	{
		m[i]=(double *) malloc((unsigned) (nch-ncl+1) *sizeof(double));
		if (!m[i]) nrerror("Fallo 2 asignación en matrix()");
		m[i] -= ncl;
	}
	for(i=nrl;i<=nrh;i++)
...
/***********************************************************
 * ASIGNACIÓN DE MEMORIA
 *	LIBERAR MEMORIA PARA UN AARREGLO 2D
 ****/
void FreeMatrix(double **m,short nrl,short nrh, short ncl,short nch)
{
	short i;
